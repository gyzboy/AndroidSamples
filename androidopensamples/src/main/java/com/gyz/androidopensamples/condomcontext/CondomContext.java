...
package com.gyz.androidopensamples.condomcontext;
...
/**
 * 使用方法:XxxClient.init(CondomContext.wrap(context, "XxxSDK"), ...);
 * 使用第三方SDK时避免相互唤醒
 *
 * 实现原理:
 * 1、开发者可主动设置一个OutboundJudge回调，方便根据需求定制拦截策略。
 * 2、避免通过此Context发出的广播启动其它应用的进程。在Android N以上，通过为非应用内广播的Intent添加FLAG_RECEIVER_EXCLUDE_BACKGROUND标志达成；
 *    在低版本Android系统中，通过添加FLAG_RECEIVER_REGISTERED_ONLY达到类似的效果。
 * 3、避免通过此Context发出的广播或请求的服务启动已被用户强行停止的应用。
 *    通过为发往应用之外的广播或服务请求Intent添加FLAG_EXCLUDE_STOPPED_PACKAGES标识达成
 *
...
public class CondomContext extends ContextWrapper {
	 ...
	 */
	public static  CondomContext wrap(final Context base, final @Nullable String tag) {
	...
	private  <T> T proceed(final OutboundType type, final Intent intent, final @Nullable T negative_value, final WrappedValueProcedure<T> procedure) {
	...
	private  List<ResolveInfo> proceedQuery(final OutboundType type, final Intent intent, final WrappedValueProcedure<List<ResolveInfo>> procedure) {
